...
CREATE TABLE activity(
  ...
  date_now              TIMESTAMP      DEFAULT NOW NOT NULL,
) USING TTL 120 seconds ON COLUMN date_now
MIGRATE TO TARGET oldactivity;
PARTITION TABLE activity ON COLUMN card_id;
CREATE INDEX aDate_time ON activity (date_now ) WHERE NOT MIGRATING;
...
CREATE VIEW minutely_entries_by_station
AS
SELECT
  TRUNCATE(MINUTE,date_time) AS MINUTE,
  station_id,
  COUNT(*) AS activities,
  COUNT(DECODE(activity_code,1,1)) AS entries,
  SUM(DECODE(activity_code,1,amount)) AS entry_total,
...
CREATE PROCEDURE FROM CLASS frauddetection.GetActivityHistory;
...
CREATE PROCEDURE GetBusiestStation AS
SELECT s.name, SUM(v.activities) as swipes, SUM(v.entries) as entries
FROM secondly_entries_by_station v
INNER JOIN stations s ON s.station_id = v.station_id
GROUP BY s.name
